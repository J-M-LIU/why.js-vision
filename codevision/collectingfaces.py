...
audio_dir=''
no_face_detected='Matthew.mp3'
start_image_capturing='Matthew.mp3'
multi_faces_detected='Matthew.mp3'
# 控制参数
error = 0
start_time = None
limit_time = 2  # 2 秒
...
while True:
    ...
    if error == 0 and face_count == 0:  # 没有检测到人脸
        ...
        # audioplayer.play_audio(os.path.join(audio_dir,no_face_detected))
        error = 1
        start_time = time.time()
    elif error == 0 and face_count == 1:  # 可以开始采集图像了
        print('[INFO] 可以开始采集图像了')
        # audioplayer.play_audio(os.path.join(audio_dir,start_image_capturing))
        break
    elif error == 0 and face_count > 1:  # 检测到多张人脸
        print('[WARNING] 检测到多张人脸')
        # audioplayer.play_audio(os.path.join(audio_dir,multi_faces_detected))
        error = 1
        start_time = time.time()
    else:
        pass
