...
from codevision.insertingcontrol4 import controltime
...
def checkactivity(videocamera=VideoCamera):
    # 传入参数
    ...
    while True:
        ...
        grabbed=vs.read()
        if grabbed and counter%timeF==0 and counter>72:
            ...
            if camera_turned == 0:
                for i in volunteer_centroids:
                    for j_index, j in enumerate(old_people_centroids):
                        ...
                        command = '%s codevision/inserting4.py --event_desc %s --event_type 1 ' \
                                  '--event_location %s' \
                                  '--old_people_id %d' \
...
def set_frame(grabbed,frame):
    if grabbed:
        ...
        globalvar.set_global_frame4(jpeg.tobytes())
        globalvar.set_is_begin(True)
    else:
        globalvar.set_global_frame4(None)