...
from codevision.insertingcontrol import controltime,stop_threads
from PIL import Image, ImageDraw, ImageFont
from codevision.oldcare.utils import fileassistant
...
def video_str():
    ...
    while True:
        ...
        grabbed = vs.grab()
        if counter % timeF == 0 and grabbed and counter > 72:
            ...
            for ((left, top, right, bottom), name) in zip(face_location_list,
                ...
                if 'Unknown' in names:  # alert
                    if strangers_timing == 0:  # just start timing
                        ...
                        if difference < strangers_limit_time:
                            ...
                            command = '%s codevision/inserting.py --event_desc %s ' \
                                      '--event_type 2 --event_location %s' \
                ...
                if name != 'Unknown' and id_card_to_type[name] == 'old_people':
                    ...
                    (smile, neural) = facial_expression_model.model_predict(roi)[0]
                    facial_expression_label = 'Neural' \
                        if neural > smile else 'Smile'
                    if facial_expression_label == 'Smile':  # alert
                        if facial_expression_timing == 0:  # just start timing
                            ...
                            if difference < facial_expression_limit_time:
                                ...
                                # insert into database
                                command = '%s codevision/inserting.py --event_desc %s ' \
                                          '--event_type 0 --event_location %s --old_people_id %d' \
                                          % (python_path, event_desc,
                                             event_location, int(name))
                                p = subprocess.Popen(command, shell=True)
                ...
                final_label = id_card_to_name[name] + ': ' \
                              + facial_expression_id_to_name[facial_expression_label] \
                    if facial_expression_label \
                    else id_card_to_name[name]
                draw.text((left, top - 30), final_label,
                          font=ImageFont.truetype('NotoSansCJK-Black.ttc', 40),
                          fill=(255, 0, 0))  # linux
                # 转换回OpenCV格式
                frame = cv2.cvtColor(np.asarray(img_PIL), cv2.COLOR_RGB2BGR)
                frame = get_f(grabbed, frame)
...
def set_frame(grabbed,frame):
    if grabbed:
        grabbed, jpeg = cv2.imencode('.jpg', frame)
        globalvar.set_global_frame1(jpeg.tobytes())
        globalvar.set_is_begin(True)
    else:
        globalvar.set_global_frame1(None)
...
def video_viewer():
    return Response(video_str(), mimetype='multipart/x-mixed-replace; boundary=frame')

@app.route('/', methods=['GET', 'POST'])
def set_choice():
    return render_template('rooml.html')
